...
int main(){
	...
	int n; cin >> n;
	set<ll> s;
	vector<vector<int>> v1(1000001), v2(1000001);
	for(int i=0; i<n; i++){
		int l, r; cin >> l >> r;
		v1[l].push_back(r);
		v2[r].push_back(l);
		s.insert((ll)l*10000000l+(ll)r);
	}
	...
	while(m--){
		int l, r; cin >> l >> r;
		if(v1[l].empty() or v2[r].empty()){
			cout << -1 << "\n"; continue;
		}
		if(s.count((ll)l*10000000l+(ll)r)>=1){
			cout << 1 << "\n"; continue;
		}
		if(*v1[l].rbegin() >= r and v2[r][0] <= l){
			cout << 2 << "\n"; continue;
		}
		cout << -1 << "\n"; continue;
	}
