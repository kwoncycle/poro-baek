...
int main(){
	...
	while(cin >> s1 >> s2){
		...
		for(int i=1; i<=n1; i++){
			for(int j=1; j<=n2; j++){
				int r1 = DP[i-1][j].size();
				int r2 = DP[i][j-1].size();
				int r3 = DP[i-1][j-1].size();
				if(s1[i-1]==s2[j-1]) r3++;
				if(r3 > r2 and r3 > r1){
		...
		string ans;
		for(auto x:DP[n1][n2]){
			while(i1 < n1 and s1[i1] != x){
				ans.push_back(s1[i1]); i1++;
			}
			while(i2 < n2 and s2[i2] != x){
				ans.push_back(s2[i2]); i2++;
			}
			ans.push_back(x); i1++; i2++;
		}
		while(i1 < n1){
			ans.push_back(s1[i1]); i1++;
		}
