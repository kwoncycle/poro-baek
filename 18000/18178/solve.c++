...
const ll hMOD = 1067675891539ll, A = 1557, Ai = 248232930467ll;
ll pA[50000], pAi[50000];
ll hashS(string &s){
    ll ans = 0;
    for(int i=0; i<(int)s.size(); i++){
        ans = (ans + (__int128)pA[i]*s[i]) % hMOD;
    }
    return ans;
}
...
int main(){
    ...
    for(int i=1; i<50000; i++){
        pA[i] = (__int128)pA[i-1] * A % hMOD;
        pAi[i] = (__int128)pAi[i-1] * Ai % hMOD;
    }
    ...
    vector<ll> hstate(state.size());
    for(int i=0; i<(int)state.size(); i++){
        hstate[i] = hashS(state[i]);
    }
    ...
    for(int i=0; i<(int)state.size(); i++){
        if(state[i].size() > 1){
            ...
            for(int j=0; j<(int)state.size(); j++){
                if(isend(state[j], u)){
                    ll H = (hstate[j] + (__int128)state[i].back() * pA[state[j].size()]) % hMOD;
                    if(!issafe(H)){
