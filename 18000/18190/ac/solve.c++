...
bool isin_A(pii p, vector<pii> &v){ // strictly in
    if((v[1]-v[0])/(p-v[0]) <= 0) return false;
    if((v.back() - v[0])/(p - v[0]) >= 0) return false;

    ll st = 1, ed = v.size() - 1;
    while(st + 1 < ed){
        ll md = (st + ed) >> 1;
        if((v[md] - v[0]) / (p - v[0]) <= 0) ed = md;
        else st = md;
    }
...
bool isin_B(pii p, vector<pii> &v){ // weakly in
    if((v[1]-v[0])/(p-v[0]) < 0) return false;
    if((v.back() - v[0])/(p - v[0]) > 0) return false;
    if((v.back() - v[0])/(p - v[0]) == 0){
        if(cross({v[0], v.back() - v[0]}, {p, p - p})) return true;
        else return false;
    }
    ...
    if(cross({v[st], v[ed] - v[st]}, {v[0], p - v[0]})){
        if(cross({v[st], v[ed] - v[st]}, {p, p - p})) return true;
        else return false;
    }
    return true;
}
...
pii lr(pii p, vector<pii> &v){
    ...
    if((v[i1]-p)/(v[(i1+1)%nv]-v[i1]) > 0){
        for(ll T = (1<<16); T >= 1; T >>= 1){
            if(T >= nv) continue;
            ll id = (i1 + T) % nv;
            if((v[id]-p) / (v[(id+1)%nv]-v[id]) > 0){
                if((v[i1] - p) / (v[id] - p) > 0) i1 = id;
            }
        }
        i1 = (i1 + 1) % nv;
        ans.X = i1;
...
int main(){
    ...
    for(ll i=0; i<n; i++) cin >> polyA[i].X >> polyA[i].Y;
    for(ll i=0; i<m; i++) cin >> polyB[i].X >> polyB[i].Y;
    // everything * 2
    ll area_B = 0;
    for(ll i=0; i<m; i++){
    ...
    while(qq--){
        ...
        //pdd pl1 = {(long double)pl.X.X/(long double)pl.X.Y, (long double)pl.Y.X/(long double)pl.Y.Y}, pr1 = {(long double)pr.X.X/(long double)pr.X.Y, (long double)pr.Y.X/(long double)pr.Y.Y};
        //cout << pl.X.X << "/" << pl.X.Y << " " << pl.Y.X << "/" << pl.Y.Y << "\n";
        pff p1 = {{p.X, 1}, {p.Y, 1}};
        pii plb = polyB[li];
        pii prb = polyB[ri];
        ...
        if(true){
            if(li > ri){
                area_minus -= B_suf[li];
                if(ri-1>=0) area_minus -= B_pre[ri-1];
            }
            else{
                if(li-1>=0) area_minus += B_pre[li-1];
                if(ri-1>=0) area_minus -= B_pre[ri-1];
            }
        ...
        if(la == ra){
            ...
            ll areall = 0;
            if((ra+1)%n <= la){
                if(la-1 >= 0) areall += A_pre[la-1]; 
                if(ra != n-1) areall -= A_pre[ra];
            }
            else{
                areall = A_suf[(ra+1)%n];
                if(la-1 >= 0) areall += A_pre[la-1];
