...
int main(){
    ...
    while(tt--){
        ...
        if(a == 2 or (p == 2 and a == 0)){
            cout << "TAK " << 1 << "\n"; continue;
        }
        vector<ll> prm; ll p1 = p-1;
        ...
        for(ll i=2; i<p; i++){
            ...
            if(!truth) continue;
            ll A = p + (a - 2*p - poww(i, m, p))%p;
            map<ll, ll> mp; ll t = 1;
            for(int j=0; (j-1)*(j-1)<=p; j++){
                mp[t] = j;
                t = t * i % p;
            }
            t = 1;
            ll ans = -1;
            ll inv = modInverse(i, p);
            inv = poww(inv, (ll)sqrt(p), p);
            for(int j=0; (j-1)*(j-1)<=p; j++){
                if(mp.find(A*t%p) != mp.end()){
                    ans = j * (ll)sqrt(p) + mp[A*t%p]; break;
                }
                t = t * inv % p;
            }
