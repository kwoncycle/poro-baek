...
struct mint{
 ll x;
 mint() { x=0; }
 mint(const ll &v) { x=(-MOD<=v&&v<MOD?v:v%MOD);
                     if(x<0) x+=MOD; }
 ...
 mint& operator+=(const mint& b)
  { if((x+=b.x)>=MOD) x-= MOD; return *this; }
 mint& operator-=(const mint& b)
  { if((x-=b.x)<0) x+= MOD; return *this; }
 mint& operator*=(const mint& b)
  { x = (int)((ll)x*b.x%MOD); return *this; }
 mint& operator/=(const mint& b)
  { return (*this) *= b.inv(); }
 ...
 friend istream& operator >> (istream &in, mint &a) {
  ll v;
  in >> v;
  a.x=(-MOD<=v&&v<MOD?v:v%MOD); if(a.x<0) a.x+=MOD;
  return in;
 }
...
template <typename T> vector<T> multiply_ntt(
...
template <typename T> vector<T> multiply(
vector<T> &v, const vector<T> &w) {
...
template <typename T> vector<T> multiply_mod(
vector<T> v, const vector<T> &w) {
 int n = 2;
 while (n < v.size() + w.size())
  n <<= 1;
...
struct Poly{
 vector<mint>v;
 Poly() { }
 Poly(mint x) { v.push_back(x); trim(); }
 Poly(const vector<mint> &V) { v=V; trim(); }
 Poly(const Poly &P) { v=P.v; trim(); }
 ...
 void trim() {
  while(v.size()>0&&v.back()==mint(0)) v.pop_back();
 }

 int lastnz() const {
  if(is_zero()) return -1;
  int ret=0;
  while(v[ret]==mint(0)) ret++;
  return ret;
 }
 ...
 Poly operator+=(const Poly &a) {
  if(v.size()<a.v.size()) v.resize(a.v.size());
  for(int i=0;i<a.v.size();i++) v[i]+=a.v[i];
  trim();
  return *this;
 }
 Poly operator-=(const Poly &a) {
 ...
 pair<Poly,Poly> div(const Poly &a) {
  assert(!a.is_zero());
  if(deg()<a.deg()) return {Poly(0),*this};
  int d=deg()-a.deg();
 ...
 Poly derivative() {
  ...
  vector<mint> ret(deg());
  for(int i=1;i<=deg();i++) ret[i-1]=(mint(i)*v[i]);
  return ret;
 }

 Poly integral() {
  vector<mint> ret(deg()+2);
  for(int i=0;i<=deg();i++) ret[i+1]=(v[i]/mint(i+1));
  return ret;
 }
 ...
 Poly pow(ll k,int n)
 {
  if(is_zero()) return k?*this:Poly(1);
  int l=lastnz();
  if(l>0){
    if(k>=(ll)(n+l-1)/l)
      return Poly(mint(0));
 ...
 mint eval(mint x) const { // evaluates in single point x
  mint res(0);
  for (int i = deg(); i >= 0; i--) {
   res *= x;
   res += v[i];
 ...
 int L, int R, vector<mint> &vec) {
  if (R - L == 1) {
   ...
   int M = L + (R - L) / 2;
   return res[v] = build(res, 2 * v, L, M, vec)
            * build(res, 2 * v + 1, M, R, vec);
  }
 }
 ...
 int ly,int ry,vector<mint> &vecx,vector<mint> &vecy){
  if (r - l == 1) {
   ...
   auto m = l + (r - l) / 2;
   auto my = ly + (ry - ly) / 2;
   auto A = (*this%tree[2*idx]).inter(
    tree,2*idx,l,m,ly,my,vecx,vecy);
   auto B = (*this%tree[2*idx+1]).inter(
    tree,2*idx+1,m,r,my,ry,vecx,vecy);
 ...
 static Poly inter(vector<mint> x, vector<mint> y) {
  int n = x.size();
  vector<Poly> tree(4 * n);
  return build(tree, 1, 0, x.size(), x).derivative().
  inter(tree, 1, 0, x.size(), 0, y.size(), x, y);
 }
...
int main(){
    ...
    P3 = P1.shift(k) + P2.shift(-k);
    for(auto i:P3.v) cout << i << " "; cout << "\n";
    return 0;
}