...
int main(){
    ...
    vector<Convex_hull> vC;
    while(v.size() > 0){
        Convex_hull CH(v.size(), v, 0);
        vC.push_back(CH);
        set<pii> st;
        for(auto i: CH.ch) st.insert(i);
        v1.clear();
        for(auto i: v) if(st.find(i) == st.end()) v1.push_back(i);
        v = v1;
    }

    int qq; cin >> qq;
    while(qq--){
        int st = 0, ed = vC.size() - 1; // st in ed out
        pii p; cin >> p.X >> p.Y;
        if(vC[0].isin(p) == 0) cout << "0\n";
        else if(vC[ed].isin(p) != 0) cout << ed + 1 << "\n";
        else{
            while(st + 1 < ed){
                int md = (st + ed)/2;
                if(vC[md].isin(p) == 0) ed = md;
                else st = md;
            }
            cout << st + 1 << "\n";
        }
