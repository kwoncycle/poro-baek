...
bool possible(int c){
    vector<Edge> edges;
    for(int i=0; i<c; i++){
        if(v[i][1] != 1 and v[i][0] != n) edges.push_back(Edge(v[i][0], v[i][1], 1, 0));
        if(v[i][0] != 1 and v[i][1] != n) edges.push_back(Edge(v[i][1], v[i][0], 1, 0));
    }
    Dinic D(n + 2, 1, n, edges);
    return D.flow >= t;
}
...
int main(){
    ...
    sort(all(v), [&](A3 l, A3 r){
        return l[2] < r[2];
    });

    int st = 0, ed = p; // st fail ed suc
    while(st + 1 < ed){
        int md = (st + ed) / 2;
        if(possible(md)) ed = md;
        else st = md;
    }
    cout << v[ed-1][2] << "\n";
