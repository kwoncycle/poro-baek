...
int main(){
    ...
    v[1].push_back(vector<int>(1,1));
    for(int i=2; i<=n; i++){
        int t = v[i-1].size();
        for(int j=0; j<t; j++){
            if(j % 2 == 0){
                v[i].push_back(vector<int>(i, 0));
                for(int k=0; k<i-1; k++) v[i][v[i].size() - 1][k] = v[i-1][j][k];
                v[i][v[i].size() - 1][i-1] = i;
                for(int k=i-2; k>=0; k--){
                    v[i].push_back(vector<int>(i, 0));
                    for(int kk=0; kk<i; kk++) v[i][v[i].size() - 1][kk] = v[i][v[i].size() - 2][kk];
                    int tmp = v[i][v[i].size() - 1][k]; v[i][v[i].size() - 1][k] = v[i][v[i].size() - 1][k+1]; v[i][v[i].size() - 1][k+1] = tmp;
                }
            }
            else{
                v[i].push_back(vector<int>(i, 0));
                for(int k=1; k<=i-1; k++) v[i][v[i].size() - 1][k] = v[i-1][j][k-1];
                v[i][v[i].size() - 1][0] = i;
                for(int k=0; k<i-1; k++){
                    v[i].push_back(vector<int>(i, 0));
                    for(int kk=0; kk<i; kk++) v[i][v[i].size() - 1][kk] = v[i][v[i].size() - 2][kk];
    ...
    vector<vector<int>> mp(nn, vector<int>(m, 0));
    int cnt = 0;
    for(int i=0; i<nn; i++){
        if(i%2 == 0)
        for(int j=0; j<m; j++) {
    ...
    for(vector<int> v1:v[n]){
        for(int i=0; i<nn; i++){
            for(int j=0; j<m; j++){
                int o = v1[mp[i][j]];
                int k = (o-1)/m;
                if(k % 2 == 1) o = k*m+m - (o-1)%m;
                if(o == n) cout << "#";
                else cout << o;
            }
            cout << "\n";
