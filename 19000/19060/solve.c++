...
int main(){
    ...
    set<int> sc;
    for(int i=0; i<n; i++) {
        if(same3[i] == i) candidate.push_back(i);
    }
    for(int c:candidate) {
        sc.insert(c);
    }
    ...
    if(r == 1) pi0 = candidate[0];
    else{ // from test, we can find that r <= 70
        vector<int> isable(r, 1);
        int t = 1250/r - 1;
        int cnt = r;
        for(int itt=0; itt<t; itt++){
            ...
            while(true){
                x = dis(gen);
                if(sc.find(x) == sc.end()) break;
            }
            for(int i=0; i<r; i++){
                if(!isable[i]) continue;
                cout << "? " << x << " " << candidate[i] << " " << x << endl;
                int j;
                cin >> j;
                if(j != x) {
                    isable[i] = 0; cnt--;
        ...
        for(int i=0; i<r; i++) if(isable[i]) pi0 = candidate[i];
    }    
    vector<int> pi(n);
    candidate = vector<int>(0);
    for(int i=0; i<n; i++){
        if(same3[i] == pi0 and i != pi0) {
            candidate.push_back(i);
        }
    }
    if(candidate.size() == 1) pin_1 = candidate[0];
    else{
        int r = candidate.size();
        int t = 1250/r - 1;
        int cnt = r;
        ...
        for(int i=0; i<r; i++) if(isable[i]) pin_1 = candidate[i];
    }
    pi[0] = pi0, pi[n-1] = pin_1;
    for(int i=1; i<n-1; i++){
        cout << "? " << pin_1 << " " << pin_1 << " " << pi[i-1] << endl;
        cin >> pi[i];
    }
    vector<int> p(n);
    for(int i=0; i<n; i++) p[pi[i]] = i;
    cout << "! ";
