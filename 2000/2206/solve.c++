...
int main(){
    ...
    dist1 = vector<vector<int>>(n, vector<int>(m, 99999999));
    dist2 = vector<vector<int>>(n, vector<int>(m, 99999999));
    for(int i=0; i<n; i++){
        string s; cin >> s;
        for(int j=0; j<m; j++){
            v[i][j] = s[j]-'0';
    ...
    if(DEBUG){
        ...
        cout << "\n";
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++) cout << dist1[i][j] << " "; cout << "\n";
        }
        cout << "\n";
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++) cout << dist2[i][j] << " "; cout << "\n";
        }
        cout << "\n";
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++) cout << visited1[i][j] << " "; cout << "\n";
        }
        cout << "\n";
        for(int i=0; i<n; i++){
            for(int j=0; j<m; j++) cout << visited2[i][j] << " "; cout << "\n";
        }
    }
    int mn = min(99999999, dist1[n-1][m-1] + 1);
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            mn = min(mn, dist1[i][j] + 1 + dist2[i][j]);
        }
    }
    if(mn > 99999990) cout << -1 << "\n";
    else cout << mn << "\n";
    //BFS(n-1, m-1)
    return 0;
}