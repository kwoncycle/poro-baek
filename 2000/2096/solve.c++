...
int main(){
	...
	int n, a, b, c; cin >> n;
	vector<int> dp(3, 0), dp1(3, 0), dp_min(3, 0), dp2(3, 0);
	cin >> dp[0] >> dp[1] >> dp[2];
	for(int i=0; i<3; i++) dp_min[i] = dp[i];
	for(int i=1; i<n; i++){
		cin >> a >> b >> c;
		dp1[0] = a + max(dp[0], dp[1]);
		dp1[1] = b + max(dp[2], max(dp[0], dp[1]));
		dp1[2] = c + max(dp[2], dp[1]);

		dp2[0] = a + min(dp_min[0], dp_min[1]);
		dp2[1] = b + min(dp_min[2], min(dp_min[0], dp_min[1]));
		dp2[2] = c + min(dp_min[2], dp_min[1]);
		for(int j=0;j<3; j++) dp[j] = dp1[j];
		for(int j=0;j<3; j++) dp_min[j] = dp2[j];
	}
	cout << max(dp[0], max(dp[1], dp[2])) << " " << min(dp_min[0], min(dp_min[1], dp_min[2]));
    return 0;
}