...
int main(){
    ...
    int m; cin >> m;
    vector<int> building(m);

    int st = 2*n + m, ed = 2*n + m + 1;
    vector<Edge> edges;
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            if(i != j){
                if(v[i].Y >= v[j].X){
                    edges.push_back(Edge(i*2+1, j*2, 1, 0));
                }
            }
        }
    }
    for(int i=0; i<n; i++){
        if(v[i].X == 0) edges.push_back(Edge(st, 2*i, 1, 0));
    }

    for(auto &i:building) cin >> i;
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(v[i].Y >= building[j]){
                edges.push_back(Edge(i*2+1, 2*n+j, 1, 0));
            }
        }
    }

    for(int i=0; i<m; i++){
        edges.push_back(Edge(2*n+i, ed, 1, 0));
    }
    ...
    for(int i=0; i<m; i++){
        ...
        vector<int> v1;
        while(cur != st){
            for(int id:D.graph[cur]){
                auto [a1,a2,a3,a4] = D.edges[id];
                if(D.edges[id].cap == 0 and D.edges[id].cur != D.edges[id].cap and D.edges[id].v != ed){
                    cur = D.edges[id].v; break;
                }
            }
            v1.push_back(cur);
        }
        reverse(all(v1));
        for(int j=1; j<(int)v1.size(); j+=2){
            cout << 1 + v1[j]/2 << " ";
        }
        cout << "\n";
    }
