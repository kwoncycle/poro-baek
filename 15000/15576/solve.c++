...
int main(){
	...
	for(int i=(int)s1.size()-1; i>=0; i--) v1[ns1 - i - 1] = s1[i] - '0';
	for(int i=(int)s2.size()-1; i>=0; i--) v2[ns2 - i - 1] = s2[i] - '0';
	
	vector<int> outs = conv(v1, v2);
	int n1 = (int)v1.size() + (int)v2.size();
	vector<int> realo(n1+4, 0);
	for(int i=0; i<(int)outs.size(); i++){
		int index = i;
		realo[i] += outs[i];
		while(realo[index] >= 10){
			realo[index+1] += realo[index] / 10;
			realo[index] %= 10;
			index++;
		}
	}
	while(!realo.empty() and *realo.rbegin() == 0){
		realo.pop_back();
	}
	if(realo.empty()) cout << 0 << "\n";
	else {
		for(int i=(int)realo.size() - 1; i>=0; i--){
			cout << realo[i];
		}
		cout << "\n";
	}
