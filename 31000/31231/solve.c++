...
int main(){
    ...
    for(int r=9; r>=1; r--){
        ...
        vector<int> fill;
        for(int i=0; i<n; i++){
            if(max(v[i],ans[i]) == r and max(v[n-1-i], ans[n-1-i]) < r){
                fill.push_back(n-1-i);
            }
            if(v[i] == r) cnt++;
        }
        for(int i:fill){
        ...
        while(cnt >= 2){
            int find = 0;
            for(int i=0; i<n; i++){
                if(ans[i] == 0 and ans[n-1-i] == 0 and v[i] < r and v[n-1-i] < r){
                    find = 1;
                    ans[i] = r; ans[n-1-i] = r;
                    break;
                }
        ...
        if(n % 2 == 1){
            if(cnt >= 1 and ans[n/2] == 0){
                ans[n/2] = r; cnt--;
            }
        }
        for(int i=0; i<n; i++){
            if(!cnt) break;
            if(ans[i] == 0 and v[i] >= r){
                ans[i] = r; cnt--;
            }
        }
        
    }
    //for(int i=0; i<n; i++) cout << max(ans[i],ans[i]); cout << "\n";
    for(int i=0; i<n; i++) cout << max(v[i],ans[i]); cout << "\n";
    return 0;
}