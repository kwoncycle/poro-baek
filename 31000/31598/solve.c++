...
int main(){
    ...
    for(int i=2; i<=1000000; i++){
        if(isprime[i]){
            for(int j=2*i; j<=1000000; j+=i) isprime[j] = 0;
        }
    }
    int tt; cin >> tt;
    while(tt--){
        ll n, a1; cin >> n >> a1;
        vector<ll> a(n);
        for(ll i=0; i<n-1; i++) cin >> a[i+1];
        a[0] = a1;
        vector<ll> plist;
        for(ll i=1; i<=n; i++){
            if(n%i == 0 and isprime[i]) plist.push_back(i);
        }

        pll rat1 = {2000000000, 1};
        for(ll p:plist){
            if(p == 1) continue;
            vector<ll> group(p, 0);
            for(ll i=0; i<n; i++){
            ...
            multiset<ll> ms;
            for(ll i:group) ms.insert(i);
            pll rat = {*ms.rbegin(), *ms.begin()};
            for(ll i=1; i<n; i++){
                //for(int j:group) cout << j << " "; cout << "\n";
                ...
                ms.insert(group[(i)%(p)]);
                pll P = {*ms.rbegin(), *ms.begin()};
                if(P.X * rat.Y < rat.X * P.Y) rat = P;
            }

            if(rat.X * rat1.Y < rat1.X * rat.Y) rat1 = rat;
        }
        ll gc = __gcd(rat1.X, rat1.Y);
        cout << rat1.X/gc << " " << rat1.Y/gc << "\n";
    }
    return 0;
}