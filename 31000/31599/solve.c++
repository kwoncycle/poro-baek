...
int main(){
    ...
    vector<vector<int>> cntv(n, vector<int>(n, 0));
    vector<vector<vector<int>>> indexv(m,  vector<vector<int>>(26));
    int ans0 = -1, ans1 = -1;
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(v[i][j] == '.') continue;
            int x = v[i][j] - 'A';
            for(int ki: indexv[j][x]) {
                cntv[ki][i]++;
                if(cntv[ki][i] >= k){
                    ans1 = i;
                    goto outt;
    ...
    if(ans1 == -1) cout << -1 << "\n";
        ...
        for(int i=ans1-1; i>=0; i--){
            int cnt = 0;
            for(int j=0; j<m; j++){
                if(v[i][j] != '.' and v[ans1][j] == v[i][j]) cnt++;
            }
            if(cnt >= k){
                ans0 = i; break;
            }
