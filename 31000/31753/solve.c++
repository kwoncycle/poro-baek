...
public:
    ...
    bool addEdge(Edge e){
        ...
        ll flow_after = flow;
        flow = flow_before;
        graph[u].pop_back(); graph[v].pop_back();
        if(flow_after > flow_before) return true;
        else return false;
    }
...
int main(){
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            if(s[i][j] == '.'){
                if((i+j)%2 == 0){
                    if(D.addEdge(Edge(id[i][j], ed, 1, 0))) ans++;
                }
                else{
                    if(D.addEdge(Edge(st, id[i][j], 1, 0))) ans++;
                }
            }
        }
    }
