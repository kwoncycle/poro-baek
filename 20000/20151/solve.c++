...
bool isintersect(LINE &l, LINE &r){ // segment
    ...
    if( (b-a)/(v-u) != 0 ){
        if(a==u or a==v or b==u or b==v) return false;
...
bool is_cross_exist(vector<LINE> &v){
    ...
    int erase = 0; // 꼭짓점 허용: 0 erase // 허용ㄴㄴ: 1 erase
    for(int i=0; i<n; i++){ 
        event.push_back({v[i][0].X, 1-erase, v[i][0].Y, i});
        event.push_back({v[i][1].X, erase, v[i][0].Y, i});
    }
    sort(all(event), [&](array<ll, 4> l, array<ll, 4> r){ // 꼭짓점 비허용시 그냥 sort
        if(l[0] == r[0] and l[1] == r[1] and l[2] == r[2]) 
            return (v[l[3]][1] - v[l[3]][0])/(v[r[3]][1] - v[r[3]][0]) > 0;
        else return l < r;
    });
    set<pll> st;
