...
struct Convex_hull{
    ...
    void rotate_CH(){
        ...
        for(int i=0, j1=0, j2=0, j3=0; i<n; i++){
            while((j1+1)%n != i and (ch[(i+1)%n] - ch[i])/(ch[(j1+1)%n] - ch[j1]) >= 0){
                j1 = (j1+1)%n;
            }
            pii u1 = ch[(i+1)%n] - ch[i];
            u1 = {-u1.Y, u1.X};
            while((j2) != j1 and (u1)/(ch[(j2+1)%n] - ch[j2]) <= 0){
                j2 = (j2+1)%n;
            }
            if(i == 0) j3 = j1;
            while((j3) != i and (u1)/(ch[(j3+1)%n] - ch[j3]) >= 0){
                j3 = (j3+1)%n;
            }
            ll A1 = (ch[j1] - ch[i])*u1, A2 = (ch[j2] - ch[j3])*(ch[(i+1)%n]-ch[i]), A3 = (ch[(i+1)%n]-ch[i])*(ch[(i+1)%n]-ch[i]);
            ll At = (__int128)A1 * (__int128)A2 / (__int128)A3, Am = (__int128)A1 * (__int128)A2 % (__int128)A3;
            if(Am*2 >= A3) At++;
            mn = min(mn, At);
            //cout << j1 << " " << j2 << " " << j3 << "\n";
        }
