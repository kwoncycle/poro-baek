...
int main(){
	...
	while(tt--){
		...
		M[2] = {c, d, a, -b};
		M[3] = {d, -c, b, a};
		for(int i=0; i<4; i++) for(int j=0; j<4; j++) M[i][j] = (m + M[i][j]) % m;
		vector<ll> t(4, 0); t[0] = 1;
		int able = 1;
		for(int i=0; i<4; i++){
			int r = -1;
			for(int j=i; j<4; j++){
			...
			for(int j=0; j<4; j++){
				...
				for(int j1=0; j1<4; j1++){
					M[j][j1] -= k*M[i][j1]; M[j][j1]%=m; M[j][j1]+=m;
				}
				t[j] -= k*t[i]; t[j]%=m; t[j]+=m; t[j] %= m;
			}
		}
		if(able)
		for(int i=0; i<4; i++){
			t[i] %= m; t[i] = (m+t[i])%m; cout << t[i] << " ";
		}
		else{
			cout << "0 0 0 0\n";
		}
