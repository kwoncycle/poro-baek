...
public:
    Dinic(ll _n, ll _start, ll _end, vector<Edge> &_Edge) : n(_n), start(_start), end(_end){
        ...
        while(true){
            reset();
            BFS();
            if(level[end] > 0x3f3f3fll) break;
            while(true){
                ll x = DFS(start, 0x3f3f3f3fll);
                flow += x;
                if(!x) break;
            }
        }
    ...
    void reset(){
        fill(level.begin(), level.end(), 0x3f3f3f3fll);
    ...
    vector<ll> mincut(){
        ...
        vector<ll> ans;
        for(int i=1; i<=n; i++){
            if(level[i] < 0x3f3f3fll) ans.push_back(i); 
        }
        return ans;
    }
};
...
int main(){
    ...
    int st = 2*n, ed = 2*n+1;
    for(int i=1; i<=n; i++){
        V.push_back(Edge(st, v[0][i], 1, 0));
        for(int j=0; j<2*n; j++){
            if(!B[v[0][i]][j]) V.push_back(Edge(v[0][i], j, 1, 0));
        }
        V.push_back(Edge(v[0][i+n], ed, 1, 0));
    }

    Dinic D(ed+1, st, ed, V);

    if(D.flow == n) cout << "possible\n";
