...
int main(){
    ...
    vector<mint> C(4001), D(4001);
    
    for(int n=1; n<=4000; n++){
        vector<mint> v0(4001), v1(4001);
        int o = 1;
        for(int i=n; i<=4000; i+=n){
            C[i] = C[i] + (mint(-1)/(mint(i/n)));
            D[i] = D[i] + (mint(o) /(mint(i/n)));
            o *= -1;
        }
    }
    ...
    for(int i=0; i<=4000; i++){
        ans[i] = (mint(C1.v[i] + D1.v[i])/mint(2)).x;
    }
    int tt; cin >> tt;
    while(tt--){
        int n, k; cin >> n >> k;
